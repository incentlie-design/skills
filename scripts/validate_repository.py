#!/usr/bin/env python3
"""Validate the public four-Skill repository contract using the standard library."""
...
REQUIRED_METADATA = {
    ...
    "owners", "tags", "dependencies", "input_contract", "output_contract",
}
...
def validate(root):
    ...
    try:
        ...
        if names != EXPECTED_NAMES:
            errors.append(f"registry must contain the exact ordered four Skills: {EXPECTED_NAMES}")
        ...
        for slice_name, required in SLICE_FIELDS.items():
            if set(definitions.get(slice_name, {}).get("required", [])) != required:
                errors.append(f"shared contract {slice_name} fields mismatch")

        routing = read_json(root / "tests/routing_scenarios.json")
        scenarios = routing.get("scenarios", [])
        if routing.get("schema_version") != 1 or len(scenarios) != 4:
            errors.append("routing scenarios must contain exactly four versioned cases")
        scenario_ids = set()
        covered = set()
        for scenario in scenarios:
            ...
            if not isinstance(scenario["stop"], str) or not scenario["stop"].strip():
                errors.append(f"{scenario['id']}: missing stop condition")
        ...
        if covered != set(EXPECTED_NAMES):
            errors.append("routing scenarios do not cover all four Skills")
    ...
    return {
        ...
        "behavior_cases_defined": cases_defined,
