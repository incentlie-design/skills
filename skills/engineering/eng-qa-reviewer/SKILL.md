...
- When the supplied subject is a PRD, TD, or architecture proposal governed by the [PRD and TD visual documentation contract](../../../docs/prd-td-visual-contract.md), check its human comprehension, Agent precision, selected-figure mapping, cross-view consistency, editable-source/render evidence, and honest `N/A` or `deferred` decisions. Verify that each supplied figure resolves a material ambiguity and agrees with prose/contracts/implementation; do not require a fixed figure set or become a second design owner.
...
- Follow the [review and handoff priority](../../../docs/review-handoff-template.md): exact subject and bounded verdict first, then material findings/evidence gaps and required owner/action. Coverage, detailed observations, exclusions, reused/stale evidence, residual risk, P3/nits, history, and appendices follow only when applicable. A clean `no_material_finding` result may be only subject, bounded conclusion, action, and one material unreviewed boundary; do not require empty sections, a zero-row ledger, or repeated project facts.
- Refer to supplied governance evidence for Session, budget, resource, and lease facts instead of inventing another schema. A clean review of one subject does not certify unreviewed integration, release, or production behavior.
