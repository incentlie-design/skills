...
An explicit user statement that the current assignment's change is merged, including “已合并”, authorizes closure, repository cleanup, and Session archival for that assignment without a second confirmation. Bind the statement to the exact assignment and candidate. If the closure scope names a dependent project that consumes the changed Skills, use `eng-workspace-governance` to refresh the Skill source `main`, reconcile its canonical registered set into that project, and record the exact source and consumer commits before archival. Route Git cleanup and synchronization writes to `eng-repo-governance`, release leases, record each cleanup, synchronization, or retention result, and then archive the closed Session. The statement does not authorize unrelated projects, assignments, or refs.
